...
export default function CalculadoraMedia(): JSX.Element {
  const [data, setData] = useState(''); // Lista de números como string (e.g., "1, 2, 3")
  ...
  const getFormulaComponents = () => {
    ...
    const sum = numbers.reduce((acc, val) => acc + val, 0).toFixed(2);
    const n = numbers.length;

    return { sum, n };
  };

  const { sum, n } = getFormulaComponents();
  ...
  return (
    <ScrollView contentContainerStyle={estilos.container}>
      <View style={estilos.header}>
        ...
        <Text style={estilos.title}>Calculadora de la Media</Text>
      </View>
      <View style={estilos.content}>
        <View style={estilos.formulaContainer}>
          <View style={estilos.formulaWrapper}>
            <Text style={estilos.formula}>x̄ = </Text>
            <View style={estilos.fraction}>
              <Text style={estilos.numerator}>{sum}</Text>
              <View style={estilos.divider} />
              <Text style={estilos.denominator}>{n}</Text>
            </View>
          </View>
        </View>
        <Text style={estilos.subtitle}>
          Donde: x̄ = media, ∑ xᵢ = suma de los valores, n = número de valores
        </Text>

        <View style={estilos.textboxContainer}>
          <Text style={estilos.textboxLabel}>Datos:</Text>
          <Text style={estilos.textboxInput}>
            {data || 'Ingrese valores (e.g., 1, 2, 3)'}
          </Text>
        </View>
        ...
        {mean !== null && (
          <Text style={estilos.result}>Media: {mean.toFixed(2)}</Text>
        )}
      </View>
    </ScrollView>
  );
